...
class NDVI_images(db.Model):
    ...
    farm: Mapped["Farm"] = relationship(back_populates="ndvi", uselist=False)
...
class Aereal_images(db.Model):
    ...
    farm: Mapped["Farm"] = relationship(back_populates="aereal", uselist=False)
...
class Farm(db.Model):
    ...
    aereal_images_id: Mapped[int] = mapped_column(ForeignKey("aereal_images.id"), nullable=False)
    ...
    def serialize(self):
        return {
            ...
            "ndvi_images_id": self.ndvi_images_id,
            "aereal_images_id": self.aereal_images_id,
        }
